...
sub handler {
    ...
    $this->{subSeparator} = $this->{separator}
      unless defined $this->{subSeparator};
    $this->{subSeparator} = '' if $this->{subSeparator} eq 'none';

    $this->{markerFormat} = $params->{markerformat};
    $this->{markerFormat} = $this->{format}
      unless defined $this->{markerFormat};
    ...
    foreach my $aweb (@websList) {
        if ( $aweb =~ /^(public|webtemplate)(current)?$/ ) {
            ...
            $seen{$aweb} = 1;
            push @list, $aweb
              if defined $allWebs->{$aweb};    # only add if it exists
        }
    }
    ...
    foreach my $aweb (@list) {
        ...
        if ( $this->{ignorecase} eq 'on' ) {
            next
              if $this->{exclude} ne ''
                  && $web->{key} =~ /^($this->{exclude})$/i;
            next
              if $this->{include} ne ''
                  && $web->{key} !~ /^($this->{include})$/i;
        }
        else {
            next
              if $this->{exclude} ne ''
                  && $web->{key} =~ /^($this->{exclude})$/;
            next
              if $this->{include} ne ''
                  && $web->{key} !~ /^($this->{include})$/;
        }
        next
    ...
    foreach my $aweb (@list) {
        ...
        next
          if $this->{subWebs} !~ /^(all|none|only)$/
              && $web->{key} !~ /$this->{subWebs}\/[^\/]*$/;
    ...
    $result = Foswiki::Func::expandCommonVariables( $result, $currentTopic,
        $currentWeb );
...
sub formatWeb {
    ...
    if ( $result =~ /\$url/ ) {
        $url =
          $this->{session}->getScriptUrl( 0, 'view', $web->{key}, $homeTopic );
    }
